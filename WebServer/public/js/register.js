window.onload = (even) => {
    //登录倒计时
    var btn = document.querySelector('.Register_box_send');
    btn.addEventListener('click', function () {
        btn.disabled = true;
        btn.innerHTML = '时间还剩下15秒';
        var time = 15;
        var timer = setInterval(function () {
            ...
            if (time == 0) {
                ...
                btn.innerHTML = '时间还剩下' + time + '秒';
            }

        }, 1000);
    });
    //登录，注册小眼睛
    var regiest_input = document.querySelector('.regiest_input');
    var password_icon2 = document.querySelector('.password-icon2');
    var password_icon1 = document.querySelector('.password-icon1');
    var flag = true;
    password_icon2.onclick = function () {
        regiest_input.type = 'text';
        password_icon2.style.display = 'none';
        password_icon1.style.display = 'block';
    }
    password_icon1.onclick = function () {
        regiest_input.type = 'password';
